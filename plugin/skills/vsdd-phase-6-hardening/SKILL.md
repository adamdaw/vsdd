...
- **Fuzz testing** (§A.4): run the campaign to its Constitution budget (minimum effort **and** coverage plateau in executions; sanitizers on; persisted corpus). Every crash is minimised, captured as a regression test, and **fixed** — no crash carried open. A budget shortfall is a *fixable* gap (spend more / scope tighter), not a release-eligible limitation. **Fuzz the WI's own owned surface, not reflexively a shared trust boundary** — a downstream WI that consumes an already-hardened boundary fuzzes *its* owned entry + the NFR slice it carries. If the owned surface splits into a fuzz-reachable entry and pipeline-internal passes, fuzz the entry and state in the pass record that the rest is hardened by mutation + green suite + purity (honest scope, not a gap).
...
- **Mutation testing:** every surviving mutant is killed by a new/strengthened test, or closed as `verified-equivalent` with committed justification (§A.8). A **non-equivalent survivor is fixed-only.** A **zero-branch plumbing file** is covered *transitively* — name the downstream mutant that kills its contribution, don't manufacture a meaningless mutant. Where the host ships **no mutation tooling**, the Constitution-designated **manual audit** discharges it: apply mutant → build → run the WI's targeted tests → confirm killed → `git checkout` revert (patch never committed; only the mutant table is), PRNG **seed = reproducible corpus** (no `Math.random`/wall-clock — the audit must itself be pure).
