...
**Creativity**
The only creativity that sets product direction and acceptance. Generates the problem framing, the quality goals, and the decision to accept or reject convergence. (The Builder's creativity — algorithm, structure, naming — shapes *how* the spec is realised, within its constraints; it does not set direction.) The Architect's judgment is the irreducible human contribution.
...
**Constraints**
No prior relationship with the artifact — fresh context on every invocation, no accumulated goodwill. Cannot pass what doesn't meet the standard; "mostly good" is not a pass. Output format is non-negotiable: `[Location — Topic]: <concrete flaw> → <proposed correction>`. No preamble, no praise, no softening — the first word is the first flaw. If genuinely no flaws exist *on a first-pass review* — zero findings ever raised, a `PASS_CLEAN` verdict (§A.7): the **invariant declaration** *"Forced to manufacture flaws."* followed by a brief statement that the artifact(s) under review meet the standard — the trailing clause names what was reviewed (*Artifact(s) meet standard* / *Documents meet standard* / *Spec meets standard*) and automation keys on the invariant prefix, not the full string (§A.7) — and nothing more. On a *re-review* where every prior finding is now fixed and no new flaw exists — a `PASS_FIXED` verdict — the output is instead *"All prior findings resolved; no new flaws."* followed by the same meets-standard statement: **not** "Forced to manufacture flaws," because flaws were genuinely found and fixed, so the reviewer was not forced to manufacture anything. **Standing gate:** invoked on every PR, not only at spec completion. Cannot approve its own prior suggestions — each review is independent.
...
**The Intent artifact (input).** Phase 1 takes a minimal **Intent**: a short statement — a few sentences is enough — carrying at least four fields: the problem or opportunity, the desired outcome, the requesting stakeholder/owner, and the acceptance condition that would make the work "done enough." It need not be formal, but those fields must be present before deliberation begins, because Gate 1 reviews the SRS against this Intent. A request missing its outcome or acceptance condition is sent back, not deliberated. The Intent is **committed to the repository (or content-hashed) before Gate 1**, so the Gate 1 pass record can pin the exact source artifact it reviewed (§A.7); an uncommitted stakeholder ticket is first captured as a committed Intent file — Gate 1 reviews that committed artifact, not a mutable external ticket. The Intent's required fields are schematised in **Appendix A §A.15**. The Intent — its four content fields *and* its **INTENT-NNN** identifier — is committed **at intake, before deliberation begins**, so it is a fixed input: deliberation works from it to produce the SRS, and Gate 1 reviews that SRS against the committed Intent (it is never a moving target). Commit-at-intake satisfies both "present before deliberation" and "pinned before Gate 1" (§A.7) at once.
...
**What the Adversary does NOT receive:** the deliberation record or ADR content. The Adversary at every gate receives the source artifact, the artifact derived from it, and the standing governing artifact the derived one must not contradict (the Constitution) — at Gate 1, the committed Intent, the committed Constitution, and the SRS — plus, from Gate 2 onward, the objective gate evidence its verdict needs; never the subjective framing (deliberation record, ADR, prior-review narrative). This boundary is not a convenience; it is the mechanism that preserves the Adversary's independence. An Adversary that knows *why* a requirement was written the way it was will probe against the specifier's reasoning rather than independently arriving at whether the conclusion is sound. The gap between what the specifier intended and what the Adversary independently finds *is* the value of the gate. Collapsing that gap by sharing the deliberation defeats the purpose.[^superpowers]
...
The weight of the SRS is keyed to the decomposition unit; the required fields for each tier are schematised in **Appendix A §A.14**:

- **Epic-sized work** — a requirement that will fan out into multiple independently-deployable work items — gets the **full SRS**: a structured requirements document along the lines of ISO/IEC/IEEE 29148.[^iso29148] Its sections: Purpose and Scope; Stakeholders (role, goals, priority); Business Requirements (each a goal with a measurable success criterion, MoSCoW-prioritised); Functional Requirements (each a user story — *As a [role], I want [capability], so that [benefit]* — decomposed into one or more EARS requirement statements per the syntax below, each its own REQ-NNN; the user story carries intent, the EARS statements the testable behaviour); Non-Functional Requirements (ISO 25010[^iso25010] quality categories, each with a measurable criterion); Constraints; Assumptions and Dependencies (with impact-if-wrong); Acceptance Criteria (Gherkin) for every *in-scope* requirement — at minimum every MoSCoW *Must* (priority, distinct from the EARS SHALL obligation keyword), plus any lower-priority requirement scheduled for this cycle; a requirement not scheduled is explicitly marked `deferred` or `rejected` (per the decision rule below) and carries no test or implementation trace; and a Requirements Traceability Matrix. The epic-level SRS is the artifact that *feeds the decomposition* into work items.
...
**Classifying the unit (intake test).** The epic-vs-work-item decision is made at intake by the **atomicity test**, applied to the request as stated — the same test the decomposition bridge later enforces, used here as the up-front classifier:

1. *Single responsibility* — can the work's responsibility be stated in one line?
2. *Independent deployability* — can it deploy without a sibling shipping first?

Both **yes** → it is a **work item** (light SRS). Either **no** — it spans several responsibilities, or cannot deploy without siblings — → it is an **epic** (full SRS, then decomposed into work items). The classification is **provisional and cheap to revise during deliberation**: a presumed work item that proves to span multiple deployable units is escalated to an epic (rewritten as a full SRS and decomposed); a presumed epic that proves atomic collapses to a single work item. A genuinely borderline case — where neither answer is confident — is treated as an **epic**, because under-decomposing (missing that the work needed splitting) is a costlier error than over-documenting a unit that turns out atomic. The cost of misclassifying early is a re-tier, not rework.

Requirements are written in **EARS format** (Easy Approach to Requirements Syntax)[^ears] — five fill-in-the-blank SHALL-based templates that reduce ambiguity, vagueness, and complexity in requirement statements. On top of EARS, VSDD applies **modal discipline**: it mandates SHALL (and admits the RFC-2119 MUST as an equivalent obligation keyword) and forbids *should*, *may*, *could*, *might*. This modal layer is a VSDD extension, not part of canonical EARS; it narrows how a requirement can be read but does not by itself remove semantic ambiguity. The five patterns:
...
**Newly discovered requirements (decision rule).** When deliberation, spec authorship, or any later phase surfaces a requirement or edge case not already in the SRS, the Architect classifies it before work continues, into exactly one of: (a) **in-scope now** — minted a REQ-NNN, scheduled this cycle, carrying its full test/implementation trace; (b) **deferred** — minted a REQ-NNN, held out of this cycle with a linked tracked item (ITEM-NNN, §A.9) and a recorded accept-the-risk rationale, carrying no trace this cycle; or (c) **rejected** — recorded in the ADR log with rationale, no REQ-NNN minted. A surfaced requirement is never silently dropped nor silently absorbed: a `deferred` or `rejected` item is an explicit, recorded decision, not an omission. An edge case first surfaced in the SDD (Phase 2) re-enters Gate 1 as a versioned SRS addendum under this same rule — it may legitimately resolve to (b) deferred, but only as a recorded decision, never as an untracked gap.

**Step 1b — SRS Review Gate (Gate 1 — SRS Fidelity Review)**

The SRS is reviewed by *both* the human and the Adversary before any technical spec is written. The Adversary receives the committed Intent (Phase 1), the committed Constitution (needed for the non-contradiction check against its quality goals), and the SRS — and no subjective framing: no deliberation record, no ADR, no architectural reasoning. This is Gate 1 in the Derivation Fidelity chain (Core Principle 9), and the gate most commonly skipped because it precedes the first design decision. The Adversary checks that every requirement is:

- **Modally conformant** — EARS-shaped: SHALL/MUST, no *should/may/could/might*. Modal discipline is necessary but not sufficient — it narrows how a requirement reads without, by itself, making it single-meaning (see the next criterion).
- **Semantically unambiguous** — even when modally clean, no requirement admits two distinct readings: terms are defined, references resolve, and an independent reviewer and implementer would agree on the behaviour it demands. This is the check modal conformance cannot perform; it is the irreducibly adversarial part of the gate (Principle 8).
...
The SRS is iterated until the Adversary can find no legitimate holes. **No technical spec authorship (Phase 2) begins until Gate 1 is cleared and the Architect signs off** — the approve-before-design gate. (The Architect's sign-off here is approval of the *gate verdict*, required on every gate record, §A.7 — not a finding sign-off: Gate 1 is a derivation-fidelity gate, so each finding is *fixed*, never signed off.) Clearing a finding means resolved-and-applied, not merely acknowledged. Enforce structurally: a pre-Phase-2 hook that checks for a committed Gate 1 pass record (Core Principle 8).
...
- **Security Clauses (security-critical paths):** A work item is **security-critical** if it touches authentication or authorisation, handles secrets or regulated/PII data, performs financial calculation, or sits on a trust boundary (parses untrusted input or crosses a privilege boundary) — a boolean tag the **Architect approves** for *every* work item before Phase 2 (the Tracker records and enforces it, but the criticality judgment is the Architect's, not the Tracker's — it determines mandatory CSDD/proof obligations): set at epic decomposition where the item came from an epic, or at intake for a standalone work item. It is never left to be decided later during implementation. For any work item carrying that tag, the Behavioural Contract includes security clauses in **CSDD (Constitutional Spec-Driven Development)** format[^marri] — Marri's pattern of encoding security constraints as versioned, machine-readable spec clauses. Each clause has a SEC-NNN identifier, a CWE reference (e.g., CWE-89 SQL Injection), a MUST/SHOULD/MAY level, an implementation pattern, and an enforcement mechanism (static analysis rule, merge blocker). The full schema — level semantics, valid enforcement mechanisms, and a worked SEC-NNN example — is in **Appendix A §A.1**. MUST-level violations reject the implementation and require a rewrite; the SHOULD and MAY level semantics are normative in §A.1. This layer is optional for non-security-critical features and mandatory, **per applicable CWE/security surface**, for those that are — each clause guards one CWE (§A.1), so it is instantiated only where such a weakness class exists (auth/authz, secrets/PII, untrusted-input or privilege-boundary handling). A work item tagged security-critical *solely* for a **financial calculation**, with no CWE-backed surface, carries the §A.3 proof obligations that financial calculations trigger but instantiates **no** CSDD clause — there is no weakness class to guard. The two obligation types are distinct: CSDD clauses attach to CWE surfaces, §A.3 proof duties attach to financial / data-integrity / safety properties, and a work item incurs whichever its surfaces and properties warrant. Empirical basis: in Marri's banking-microservices case study, constitutional security constraints were associated with a 73% reduction in security defects versus unconstrained AI generation, with no reported velocity degradation.[^marri] It is a single case study; treat the figure as indicative, not a guaranteed general result.
...
- **Verification Tooling Selection:** Based on the language and the properties to be proven, the Builder selects the appropriate formal verification stack (Kani for Rust, CBMC for C/C++, Dafny, TLA+ for distributed systems, etc.) and identifies any constraints these tools impose on code structure. This happens *now*, not after the code is written, because tool constraints are architectural constraints. Where tooling selection is uncertain, a **Research artifact** is produced first — a spike that validates the chosen tool against a representative property before the full verification architecture commits to it. The Research artifact records spike results, the tool decision, and reasons alternatives were rejected; it is committed to the repository alongside the Constitution and Spec.[^speckit] Its fields, owner, and acceptance are defined in **Appendix A §A.6** — which also fixes its **admissibility split**: Gate 2 receives only the objective fields (representative property, result, evidence, decision, and the rejected-alternatives *list*), not the *rationale* behind each rejection, which is withheld human record like the deliberation narrative.
- **Property Specifications:** For **every** Prove-classified property (§A.3), the Builder drafts the actual formal property definition (e.g., Kani proof harnesses, Dafny contracts, TLA+ invariants) alongside the behavioural spec. These aren't implementation — they're the formal expression of what the spec already says in natural language, a second, mathematically precise encoding of the requirements. Where a property genuinely cannot yet be expressed in the selected tooling, that is not a silent skip: it requires a §A.6 research/escalation record demonstrating the inexpressibility, which Gate 2 reviews — there is no "where possible" escape hatch, since a Prove property with no drafted specification and no such record fails Gate 2.
...
- **Contradictions with the Constitution** — a Spec clause that violates a governing principle, architectural rule, or quality goal (Principle 1); the remedy is a Constitution amendment with rationale, never a silent per-Spec exception
...
3. **Test Quality:** Are the tests actually testing what they claim? Are there tests that would pass even if the implementation were subtly wrong? (Tautological tests, tests that mock too aggressively, tests that assert on implementation details rather than behaviour.) **A test finding that affects spec/test fidelity** — tautological, over-mocked, or not a faithful derivation of the acceptance criteria — is a **derivation-fidelity finding** (the same class Gate 3 owns) and is **fixed-only**, never signed off; Pass 2 acceptance is reserved for *non-fidelity* test findings (flakiness, naming, test runtime). Catching such a gap at Pass 2 rather than Gate 3 does not convert it into an acceptable quality finding — a fidelity gap cannot be laundered through the gate that happens to surface it.
...
**Loop Termination Signal — Architectural Escalation:** A *cycle* is one return-and-re-review of a given work item or requirement through the feedback loop. If the loop cycles three times on the same work item (same ITEM-NNN) or the same requirement (same REQ-NNN / SEC-NNN) — three successive review attempts (FAIL records) each returning a finding that resolves to that same item or requirement, whether or not the specific line moves — the issue is architectural, not implementational. Three failed fix attempts without convergence is the signal to *stop fixing* and escalate to the Architect: the spec or architecture is wrong, not the implementation. **Counting is per affected identifier:** each FIND-NNN records the REQ-NNN / SEC-NNN / ITEM-NNN it maps to (§A.8); a finding spanning several identifiers counts a cycle against *each*; escalation fires when any single identifier reaches three failed cycles, even if no individual finding recurred verbatim. The Architect then decides whether to revise the spec, revise the architecture, or de-scope. But a **derivation-fidelity gap** (Gates 1–3, Gate 4 Pass 1) is never "accepted" as a passing limitation, and never ships as §A.13 non-convergence either: the gap is between two artifacts the Architect controls, so closing it is always within their power — it is fixed by revising the owning artifact, or by de-scoping the requirement (itself a fix to the SRS), never signed off. The §A.13 Documented-Limitation route is reserved for what *cannot* be closed by revision — an undischarged Prove property or an under-envelope concurrency bound, both blocked by external tooling limits (§A.3). A CSDD MUST-security violation (§A.1) likewise cannot be accepted as converged; it is fixed or compliance-verified, never a pass. Attempting a fourth fix without this escalation is almost always slower than surfacing the architectural problem directly — and the fix is nearly always wrong.
...
All formal verification and fuzzing results feed back into Phase 5 if issues are found. Phase 6 checks are CI gates — formal proofs, fuzz results, every mutation survivor triaged (killed or justified as equivalent), and purity boundary audit outcomes must all pass as structural prerequisites before Phase 7 convergence can be declared. These are hook-enforced, not self-certified (Core Principle 8). This is **Gate 5 — Verification** (formal proofs, fuzzing, mutation testing, security hardening, and the purity-boundary audit) — predominantly deterministic tooling, with two recorded manual exceptions (below). Its core adversary is deterministic: the prover, fuzzer, and mutation-tester are maximally-hostile reviewers that cannot be flattered or fatigued. Formal verification *is* adversarial review executed by tooling — the point where Principle 8 (external enforcement) and Principle 9 (derivation fidelity) become the same act. Two parts of Gate 5 require human judgment rather than pure tooling in the baseline — the equivalent-mutant justification (mutation testing) and the purity-boundary audit's conclusions — and these follow the Principle 8 manual-exception rule: committed, Architect-signed evidence, not self-certification. (A project's Constitution may designate *additional* manual exceptions where a required hardening tool admits no deterministic verdict; each follows the same rule. The baseline is two; the set is Constitution-extensible, so the README speaks of "recorded manual exceptions" rather than a fixed count.) The remainder (proof discharge, fuzz crash-freedom, mutant kills) is deterministic tool output. The gate passes only when every Phase 2b property has a successful proof result or a permitted bounded-concurrency result; an undischarged Prove property blocks the gate (it is non-convergence, §A.3), never a passing result.

---

#### **Phase 7 — Convergence (The Exit Signal)**

Convergence is declared **per dimension by committed evidence**, never by a reviewer's sense that an artifact is "done." A dimension has converged when its gate carries a committed pass record (schema in **Appendix A §A.7**) and **every finding it produced — whether raised by an adversary or by tooling (static analysis, fuzzer, mutation tester) — has been dispositioned**, each captured in a finding/disposition record (**Appendix A §A.8**). *What counts as a disposition depends on the gate* (§A.7): a **derivation-fidelity finding** — at Gate 1, Gate 2, Gate 3, or Gate 4 Pass 1 — is *fixed*, never signed off, because a gap between a derived artifact and its source is corrected, not accepted; a **quality, security, process, dependency, or tool finding** — Gate 4 Pass 2, and Gate 5's analyzer/tool findings — is either resolved-and-applied or given an explicit, committed Architect sign-off (*accept-with-rationale*, or *accept-risk-deferred*: a tracked follow-up the Architect consciously accepts shipping with the item open — a bare deferral that does not name the accepted risk and link a follow-up item is not a disposition). Nothing proceeds past a gate on an open finding, **at any severity** — there is no severity cutoff that lets a defect through unexamined. Severity orders the work and informs the Architect's decision; it never substitutes for it.

**Sign-off is available only on quality/tool findings, and two classes are exempt even there:** a CSDD MUST-level security violation (Appendix A §A.1) and an undischarged Prove-classified property (§A.3). Neither can be signed off, deferred, or shipped as converged — the MUST violation is fixed or compliance-verified by an alternative the reviewer confirms satisfies the clause; the Prove property is proven, or re-architected until provable (an undischarged Prove property is honest non-convergence, recorded in a §A.13 Documented-Limitation record, never a waiver). For the quality/tool findings where sign-off *is* available — SHOULD-level security clauses, analyzer findings, and dependency CVEs among them — an explicit committed sign-off is a valid disposition; derivation-fidelity findings (Gates 1–3, Gate 4 Pass 1) are fixed, not signed off, as above. (A MAY-level clause produces an advisory note, not a finding, and needs no disposition — §A.1.)

The disposition policy, the fuzz budget, and the coverage floor referenced below are fixed in the Constitution.

| Dimension | Convergence criterion (committed evidence — automated where scriptable; recorded adversarial/manual judgment with rationale where not, per Principle 8) |
| --- | --- |
| **Requirements (SRS)** | Gate 1 pass record committed; every finding *fixed* (a derivation-fidelity gap is corrected, not signed off, §A.7); none left open. |
| **Spec** | Gate 2 pass record committed; every finding *fixed* (a derivation-fidelity gap is corrected, not signed off); none left open. |
| **Tests** | Two criteria, completing at different times. **(a) Test derivation** (at Gate 3): pass record committed; every finding *fixed* (a derivation-fidelity gap is corrected, not signed off); none open; every behavioural-contract item and every `automated` Gherkin scenario maps to an executable test, while each `environment-visible` / `person-confirmed` scenario maps to a manual-acceptance record (§A.10) — which Gate 3 accepts in `planned` state. **(b) Manual acceptance executed**: each such §A.10 record reaches `executed` against the hardened build in Phase 6 — verified at the convergence roll-up, *not* at Gate 3. The Tests dimension converges only when both (a) and (b) hold. (Mutation-survivor triage converges under Verification, since it requires implemented code.) |
| **Implementation** | Gate 4 pass record committed (both passes dispositioned, §A.7); every Pass 1 spec-fidelity finding *fixed* (not signed off) and every Pass 2 quality/security/process/dependency finding fixed or carrying an explicit committed Architect sign-off (no severity auto-pass); static analysis run; test coverage meets the Constitution's coverage floor. |
| **Verification** | Gate 5: every finding dispositioned, none left open; every Phase 2b property has a successful proof result or a permitted bounded-concurrency result (an undischarged Prove property is recorded as explicit non-convergence that blocks Zero-Slop, never a passing result); the fuzz campaign ran to its agreed budget with no open crash; security-hardening checks pass at their Constitution thresholds, or a breach carries a committed disposition — fixed, or signed off where the clause permits (a CSDD MUST or undischarged Prove breach is non-waivable, §A.1/§A.3); every mutation survivor killed or justified as equivalent; purity-boundary audit clean. |
...
**Maximum Viable Refinement** is reached when every dimension has converged. The software meets the **Zero-Slop** bar — defined as *zero **open** findings, with every accepted risk enumerated in the convergence roll-up* (§A.7), which the convergence table above expresses row by row — meaning every line of *production behaviour* traces to an SRS requirement (REQ-NNN, with the SEC-NNN clause instances that govern it where applicable) through its spec contract, is covered by an executable test or an executed manual-acceptance evidence record (§A.10), and has survived adversarial scrutiny, with every §A.3 Prove-classified property formally proven (or a permitted bounded-concurrency result recorded). Supporting code that Phase 3 exempts — proof harnesses, build/config/infra scaffolding, governance code — traces instead to the Constitution or the verification artifact it serves, not to a REQ-NNN.

**Zero-Slop means no *un-dispositioned* slop, not zero accepted risk.** Every `accept-risk-deferred` finding (§A.8) is a conscious, Architect-owned exception enumerated in the convergence roll-up (§A.7) — a known item shipped with eyes open, never hidden debt. A dimension carrying such a deferral has still converged, but the roll-up names the carried risk explicitly; what Zero-Slop forbids is an *open*, un-dispositioned finding, not a recorded accepted one. (The two non-waivable classes — a CSDD MUST violation and an undischarged Prove property — cannot be deferred at all; they are fixed, compliance-verified, or proven, or the dimension does not converge.)

**Convergence requires evidence, not declaration.** Convergence is a deterministic roll-up, not a fresh adversarial review — the adversarial reviews are Gates 1–5. It is declared by committing a **convergence record** (a roll-up, §A.7) that cites the five gate pass records, every finding's disposition record (with accepted risks enumerated), and every `executed` manual-acceptance record (§A.10) — asserting that all are committed, no finding is left open, every required manual-acceptance record is executed, and **no Documented-Limitation (§A.13) is open**. A roll-up exists only at full convergence; an artifact carrying an open limitation is released, if at all, via a Non-Converged Release Authorization (§A.16), not a roll-up. The *"Forced to manufacture flaws"* declaration is a property of a clean adversarial gate pass (`PASS_CLEAN`, §A.7), not of convergence — convergence itself carries no adversarial verdict, only the roll-up assertion. A convergence claim unaccompanied by the roll-up record is an assertion without evidence and does not satisfy Principle 11.

**Two terminal states.** Full convergence (the roll-up above) is one. The other is an explicit **non-converged release** (§A.16): the Architect's labelled, non-delegable decision to ship an artifact carrying an open Documented-Limitation (§A.13), which does *not* carry the verification guarantee and is never represented as a convergence roll-up. A CSDD MUST violation and a derivation-fidelity gap can take *neither* route — they are fixed (or compliance-verified), full stop. Only an undischarged Prove property or under-envelope concurrency bound — blocked by external tooling limits, not by a fixable gap — can reach the non-converged release path, and only after proving, re-architecting, *and* de-scoping the feature have all been shown genuinely infeasible (§A.16). De-scoping is the default; shipping unverified is the last resort, and routine resort to it means the verification architecture or the scope is wrong.
...
```
Intent → [Deliberation — non-gated human evidence, outside the traceability chain] → SRS (Clarified Requirements, REQ-NNN)
  ↓ [Gate 1: Adversary — SRS vs Intent]
Work Item (light-SRS slice) — decomposition of the epic SRS, before Phase 2
  ↓
Spec / SDD (per work item): Constitution + Behavioural Contract → Verification Property
...
**Ecosystem positioning:** On Piskala's three-level SDD rigor taxonomy[^piskala], VSDD sits at **L2+ (Spec-Anchored with formal hardening approaching L3)**. In Piskala's taxonomy, L2 (Spec-Anchored) means the specification is maintained alongside the code throughout the lifecycle, with any behaviour change updating both — the spec as living documentation. *VSDD's* instance of L2 adds to that baseline: every change requires a versioned addendum, and full REQ→test→implementation→review traceability is enforced (these are VSDD's additions, not part of Piskala's spec-anchored definition). VSDD further adds Phase 6 formal hardening (Kani/TLA+) that pure L2 frameworks do not have, without going full L3 (Spec-as-Source, where code is generated directly from the spec and not manually edited).

**Calibration heuristic (VSDD, informed by Piskala's taxonomy[^piskala]):** Before choosing a rigor level, answer one question: *"How many times in the past year did someone ask about requirements-to-test traceability?"* Zero times — L1 (Spec-First) is sufficient. Once — move to L2 (VSDD). More than once, or if an audit is likely — L2 (VSDD) plus CSDD security clauses for the security-critical paths. **Fallbacks where the year-count does not apply:** for a greenfield project or team with no prior year to count, substitute the *forward* test — any of {a security-critical or regulated path, an intended multi-year maintenance life, or a contractual/compliance audit on the horizon} puts the project at L2 (with CSDD clauses for the security-critical paths); none of those, and a throwaway or short-lived scope, sits at L1. When in doubt between two levels, choose the higher: under-ceremony is discovered late and expensively, over-ceremony early and cheaply.
...
[^ears]: Mavin, A. (2009). EARS (Easy Approach to Requirements Syntax). IEEE International Requirements Engineering Conference. Originally developed at Rolls-Royce aerospace; its SHALL-based templates reduce ambiguity, vagueness, and complexity in requirement statements (the paper's named problem classes). The modal-discipline framing — forbidding *should/may/could/might* — is VSDD's extension, not a Mavin claim.
