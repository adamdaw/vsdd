...
2. **Architectural Rules** — patterns all Specs must honour: layering, where I/O and persistence route, the pure-core / effectful-shell boundary, naming conventions. An **isolation or no-naming rule** here prohibits *coupling* — a variant-specific branch or identifier in an executable path — **not lexical occurrences** in comments or docstrings; wording such a rule "absolute" invites a reviewer split on comments. Where a capability-addition has a genuine need no existing seam serves, a **new generic, language-/variant-agnostic seam** is permitted provided it names no specific consumer and regresses no other (measured); the no-naming ban on executable paths stays absolute.
3. **Quality Goals & Thresholds** — ISO 25010 goals, each with a measurable target; the **coverage floor**, specified completely enough to be objectively checkable (Principle 8) — the metric (line / branch / statement), the tool and config, the included paths and exclusions, the threshold, the rounding rule, and the evidence command that produces the number; a bare percentage is not a checkable floor; where code under a coverage pin runs **off the main thread** (a worker, subprocess, or compiled artifact) so in-process instrumentation cannot see it, the SDD must supply **main-thread unit anchors** exercising that logic in-process — an off-thread architecture makes a pin *unmeasured*, never *unmeetable* or waivable; the **severity & disposition policy** (what separates a wording nitpick from a substantive finding, per the convergence rule).
...
**Amendment process.** A Spec that would violate the Constitution triggers an amendment, never a silent exception (Principle 1). An amendment is: proposed with rationale → adversarially reviewed for *whether it weakens a guarantee* → approved by the Architect → committed as a new, dated version with the rationale retained. An amendment that weakens a guarantee is **blocked** unless the Architect explicitly accepts the weakening with committed rationale; an amendment that would weaken a non-waivable baseline — the §A.1 MUST floor or the §A.3 Prove baseline — cannot be accepted at all. Amending out or scoping down a MUST clause is permitted *only* on the inapplicability showing of §A.1 — committed evidence that the guarded weakness class has no reachable surface — which the governance-gate Adversary reviews specifically; a relaxation that leaves a reachable surface under-protected is a forbidden weakening, not an inapplicability amendment. When an amendment (or a ratified clarification) fixes a **cross-cutting behaviour** — how a reference *kind*, an edge, or a shared mechanism resolves — the amendment step **sweeps the SRS/SDD for sibling requirements that share the pattern** and ratifies them together (or records why they differ); a clarification ratified for one requirement but not its siblings surfaces as a gap at a later work item's gate (Phase 5).
...
- **Target** — the pure function, parser, or state machine under test (the effectful shell is not the fuzz target). The target is the **work item's own owned surface**, not reflexively a shared trust boundary: a security-critical WI that *owns* a trust boundary fuzzes it; a downstream WI that only *consumes* an already-hardened boundary fuzzes its own owned entry and the cross-cutting NFR slice it carries — re-running the upstream corpus exercises none of the WI's code. Where the owned surface splits into a fuzz-reachable entry and passes reachable only inside a pipeline, fuzz the reachable entry and **state in the pass record** that the rest is hardened by the mutation audit + green suite + purity audit (honest scope, not a gap).
...
**Mutation coverage of the owned surface** (paired with §A.2's mutation budget). A **zero-branch plumbing file** is covered **transitively** — record which downstream mutant kills its contribution rather than manufacturing a meaningless mutant on branchless code. Where the host ships **no mutation tooling**, the Constitution-designated **manual mutation audit** discharges Gate 5 deterministically: apply mutant → build → run the WI's targeted tests → confirm killed (non-zero exit) → `git checkout` revert; the throwaway patch is never committed (only the mutant table in the pass record is), and the PRNG **seed is the reproducible corpus** (no `Math.random` / wall-clock — a purity requirement the audit itself must satisfy).
...
**External-behaviour spikes.** The same artifact governs an **external-behaviour spike** — the objective investigation a work item runs when its behaviour **depends materially on the behaviour of a component it does not own** (a third-party library, a framework or host, an external service, a peer module) — run **before SDD authoring** to establish what that component actually does, so the SDD's reliances (§A.11) trace to evidence rather than assumption. Such an item is flagged at decomposition (§A.9); its spike is mandatory. A spike for a **cross-cutting** capability must trace **every keyspace / entry point the capability touches**, not just the first — a capability folded through only the obvious site produces same-input-works / variant-fails bugs — and the SDD enumerates those sites as the seam surface. Expect a **long cold Gate-2 loop** for such externally-dependent items; read the finding-count *trend* (a steady decline is convergence, not thrash), not the round number.
...
**Verdict is set by gate history, not the reviewer's phrase.** Under genuine production independence (§A.17) a re-reviewing Adversary has **no knowledge of prior rounds** — so on a clean re-review it emits the `PASS_CLEAN` invariant (*"Forced to manufacture flaws."*), because nothing told it there were findings to resolve. The **record-level verdict is therefore set by the gate's committed history, not by the reviewer's words**: a gate that had open findings in a prior round closes `PASS_FIXED` (its *Resolved findings reviewed* field cites them) even though the cold reviewer's own output is the `PASS_CLEAN` invariant. `PASS_FIXED`'s *"All prior findings resolved"* is the phrasing the **Architect records** when closing the gate, not a phrase a context-free reviewer can author — only a same-window *memory-reset* reviewer (weaker than the §A.17 ideal) could. Automation keys the record verdict on the gate's finding history, never on which invariant sentence the reviewer returned.

**Cold-loop termination — fresh diff vs byte-stable artifact.** A derivation-fidelity gate converges on a clean cold pass by a reviewer who did not run the prior pass — but how many rounds that takes turns on whether the reviewed artifact is *changing*. A **fresh or large diff** must expect a multi-round cold loop: each fix can induce drift that exposes the next residue, so a single clean pass on a just-changed artifact is not convergence — re-review after **every** fix, context-free. A **byte-stable** artifact — unchanged by pinned SHA since a *distinct* reviewer last cleared it — converges on **one** clean cold pass; forcing further rounds against an unchanged artifact is loop-thrash hunting phantom drift. The discriminator is the pinned SHA already in *Artifact(s) reviewed*: the record states whether the implementation is byte-stable since the prior clear, so termination is checkable, not asserted.
...
**A disposition is not self-certifying.** When an Architect disposition *re-characterises scope* or routes a finding — as opposed to a plain code fix — the next adversarial round must read the disposition's artifacts *together* (e.g. SRS + SDD) and re-validate their consistency. A disposition can itself be internally incomplete — it can contradict an un-amended artifact the Architect reasoned past from intent — and the cold, evidence-isolated Adversary is what catches that. Re-run the cold adversary **after a scope-affecting disposition**, not only after a code fix.
...
**Completion links (beyond plain dependencies).** A decomposition may record a **completion link** distinct from a dependency edge: *ITEM-B completes a sub-case of ITEM-A's REQ-NNN*. ITEM-A still **owns** the requirement, but one sub-case cannot be satisfied until ITEM-B's capability exists (e.g. typing a parameter argument needs a later external-type-detection item). This is neither ownership transfer (ITEM-A owns the REQ) nor a plain dependency (ITEM-A ships first, carrying the sub-case as a §A.11 named committed fallback until ITEM-B lands) — record it as *"deferred completion, not co-ownership."* A bare "disclose and defer" that names no completing item is not a disposition.

**External-dependency flag.** Decomposition flags an **externally-dependent** item — behaviour depending materially on a component it does not own (a third-party library, a framework or host, an external service, a peer module) — so its §A.6 external-behaviour spike is scheduled before SDD authoring and its long cold Gate-2 loop is expected. A decision to *defer* a cross-cutting property to a later item must name the specific **axis** deferred (e.g. lexical-keyword casing vs identifier-resolution folding); conflating axes leaves a real bug latent in the current item.
...
| **External / unowned-component reliances** | where the item builds on behaviour it does not own | Every behavioural claim about a component **outside the work item's owned surface** — a third-party library, a framework or host API, an external service, a peer module, or the language runtime — that the Gate-2 Adversary **cannot verify from the admitted bundle** is recorded here and marked **`[Gate-3 reliance]`**, never asserted as a settled design pin. Gate 2 checks only that each reliance derives from the SRS or an admitted §A.6 spike (not invented); Gate 3, whose tests run against the real host, is where it is confirmed, revised, or found **FALSE** — a reliance resolving FALSE (the host does *less*, or *other*, than assumed) is a normal Phase-5 loop-back (an Architect-approved SDD clarification), not a defect. Absent where the item owns its whole surface. |
...
**Unowned-component design pins — the Gate-2 evidence-isolation boundary (§A.17).** The Gate-2 Adversary is isolated from code the work item does not own — a third-party library, a framework or host API, an external service, a peer module — by design, so it can check the SDD against the *SRS and Constitution* but **cannot validate an unowned component's design choice and must not invent or affirm one**. A behavioural claim about code the work item does not own is admissible at Gate 2 *only* as a marked `[Gate-3 reliance]` traceable to the SRS or an admitted §A.6 spike; an **unmarked design pin on unowned behaviour with no SRS basis is a Gate-2 finding** — left unmarked, the Adversary reasons it "on its own merits" and can affirm a divergence from the real component that only Gate 3 catches, costing a full Phase-5 loop-back. Where the work item is modelled on a *peer* implementation of the same component (e.g. a sibling adapter, an existing caller of the same library), the Architect MAY admit that **peer** code (never the work item's own) to the Gate-2 bundle, so the reliance is sanity-checkable without breaking spec-fidelity isolation.

**Conservatism vs silent requirement reduction (committed fallback).** Leaving an **unambiguous** required reference unresolved is an unsanctioned requirement reduction — a `fidelity` finding (§A.8), not a design choice; leaving a **genuinely ambiguous** reference unresolved is legitimate conservatism. The SDD distinguishes the two, and where a SHALL depends on an unconfirmed reliance it carries a **named committed fallback** — a construction engaged only if the reliance's Gate-3 fixture is red — so the requirement always has a satisfaction path. Only the Architect, via an SRS amendment, may actually reduce a requirement; the SDD may not do it silently.
